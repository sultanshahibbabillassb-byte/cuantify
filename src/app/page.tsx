...
export default function LoginPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0f',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Plus Jakarta Sans', sans-serif",padding:'24px'}}>
      <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@700&family=Plus+Jakarta+Sans:wght@400;500;600&display=swap" rel="stylesheet"/>
      <div style={{background:'#111118',borderRadius:'20px',padding:'40px',width:'100%',maxWidth:'400px',border:'1px solid #ffffff0d',boxShadow:'0 8px 60px #000000aa'}}>
        ...
        <button onClick={handleGoogle} style={{width:'100%',padding:'12px',borderRadius:'10px',border:'1px solid #ffffff16',background:'#1c1c2e',display:'flex',alignItems:'center',justifyContent:'center',gap:'10px',fontSize:'14px',fontWeight:'600',color:'#eeeef5',cursor:'pointer',fontFamily:'inherit',marginBottom:'16px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px'}}><div style={{flex:1,height:'1px',background:'#ffffff0d'}}/><span style={{fontSize:'11px',color:'#50506e'}}>atau</span><div style={{flex:1,height:'1px',background:'#ffffff0d'}}/></div>
        ...
        <button onClick={handleEmailAuth} disabled={loading} style={{width:'100%',padding:'13px',borderRadius:'10px',border:'none',background:loading?'#c8ff0060':'#c8ff00',color:'#0a0a0f',fontSize:'14px',fontWeight:'700',cursor:loading?'not-allowed':'pointer',fontFamily:"'Space Grotesk', sans-serif"}}>{loading?'Loading...':isSignUp?'Daftar':'Masuk'}</button>
        <div style={{textAlign:'center',marginTop:'16px',fontSize:'13px',color:'#9090b8'}}>{isSignUp?'Sudah punya akun? ':'Belum punya akun? '}<span onClick={()=>setIsSignUp(!isSignUp)} style={{color:'#c8ff00',cursor:'pointer',fontWeight:'600'}}>{isSignUp?'Masuk':'Daftar'}</span></div>
      </div>
    </div>
  )
}
